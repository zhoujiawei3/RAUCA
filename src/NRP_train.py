...
def train(device,hyp, opt,log_dir,logger):
    ...
    texture_param = (np.ones((1, faces.shape[0], texture_size, texture_size, texture_size, 3))*color).astype('float32') #face.shape[0]是面的数量
    ...
    # saved_state_dict = torch.load('.pth')  # 原始的参数字典
    # new_state_dict = {}
                ...
                try:
                    ...
                    #             os.path.join(log_dir, "训练图像切割图像.png"))
                        ...
                        Image.fromarray(np.transpose(255*output.data.cpu().numpy(), (1, 2, 0)).astype('uint8')).save(
                            os.path.join(log_dir, 'outpurpng'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/nrp.yaml', help='data.yaml path')
    ...
    parser.add_argument('--datapath', type=str, default='../data/NRP_training_data/',
                        help='data path')
