...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        # mask_renderer.renderer.renderer.camera_up = [0, 0, 1]
        # mask_renderer.renderer.renderer.background_color = [1, 1, 1]
        start_load=time.perf_counter() #一个魔术方法，定义了之后就可以直接class[index]获得想要的数据，也可以切片，
        ...
        # scale=0.43
        # eye=[cam_trans[0][0]*scale,cam_trans[0][1]*scale,cam_trans[0][2]*scale]
        # self.mask_renderer.renderer.renderer.eye = eye
        ...
        end=time.perf_counter()#再根据物体的片图结合之前输入的角度和距离信息就可以得到imgs_pred
        # Image.fromarray(np.uint8(255 * imgs_pred.squeeze().cpu().data.numpy().transpose(1, 2, 0))).show()
        ...
        color_ref_path=os.path.join("../../data/NRP_training_data/differentColor",self.color_name,filename)
...
def load_image(self, index):
    ...
    return img, (h0, w0), img.shape[:2], (veh_trans, cam_trans)  # img, hw_original, hw_resized ，h0，w0是原来的大小，h，w是resize成正方形后的大小
